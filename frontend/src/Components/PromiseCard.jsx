...
const PromiseCard = ({ politician, onShare, onReport }) => {
  const { t, translatePolitician, translateCategory } = useLanguage();
  const cardRef = useRef(null);
  const p = translatePolitician(politician);
  const rate = politician.neta_score ?? politician.fulfillment_rate ?? 0;
  const breakdown = politician.score_breakdown || {};

  const tone = getTone(rate, t);
      ...
      <div className="p-5">
        <div className="flex gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h3 className="truncate text-xl font-black text-white">{p.name}</h3>
            <p className="text-sm text-gray-400">{p.party}</p>
        ...
        <div className="mt-5 grid grid-cols-[120px_1fr] gap-4">
          <div className="rounded-lg border border-white/10 bg-black/30 p-4 text-center">
            ...
            <div className="text-[11px] font-bold uppercase tracking-[0.16em] text-gray-400">{t('netaScore')}</div>
          </div>
          <div className="space-y-3">
            <Meter label={t('meterPromises')} value={breakdown.promise_delivery ?? politician.fulfillment_rate} />
            <Meter label={t('meterProgress')} value={breakdown.citizen_progress ?? 0} />
            <Meter label={t('meterCondition')} value={breakdown.area_condition ?? 0} />
          </div>
        </div>

        <div className="mt-5 grid grid-cols-3 gap-2 text-center">
          <MiniStat icon={CheckCircle} label={t('statKept')} value={politician.promises_kept} tone="text-emerald-300" />
          <MiniStat icon={Clock} label={t('statRunning')} value={politician.promises_in_progress} tone="text-yellow-300" />
          <MiniStat icon={XCircle} label={t('statBroken')} value={politician.promises_broken} tone="text-red-300" />
        </div>
        ...
        <div className="mt-5 rounded-lg border border-white/10 bg-white/5 p-3">
          ...
          {politician.recent_reports?.length ? (
              ...
              {politician.recent_reports.slice(0, 2).map((report) => (
                <div key={report.id} className="rounded-md bg-black/25 p-2 text-xs text-gray-300">
                  <div className="font-bold capitalize text-white">
                    {translateCategory(report.category)} | {report.area}
                  </div>
            ...
            <p className="text-sm text-gray-400">{t('noReportsYet')}</p>
          )}
        </div>
...
function getTone(rate, t) {
  if (rate >= 75) {
    return {
      label: t ? t('publicFavorite') : 'Public favorite',
  ...
  if (rate >= 55) {
    return {
      label: t ? t('inTheFight') : 'In the fight',
  ...
  return {
    label: t ? t('pressureRising') : 'Pressure rising',
